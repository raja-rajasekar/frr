...
def test_peer_group_graceful_shutdown_hierarchy():
    ...
    # Verify both R1 and R2 receive GSHUT community
    success, result = topotest.run_and_expect(
        lambda: check_prefix_gshut_enabled(r1, "10.3.3.3/32") and
                check_prefix_gshut_enabled(r2, "10.1.1.1/32"),
        True, count=12, wait=5
    )
    assert success, "R1 and R2 should receive GSHUT community after peer-group-level enable"
