...
static int updgrp_policy_update_walkcb(struct update_group *updgrp, void *arg)
{
	...
	UPDGRP_FOREACH_SUBGRP (updgrp, subgrp) {
		...
		if (changed) {
			...
			frrtrace(5, frr_bgp,
				 upd_announce_route_on_policy_change, 0,
		...
		if (def_changed) {
			...
			frrtrace(5, frr_bgp,
				 upd_announce_route_on_policy_change, 1,
				 updgrp->id, subgrp->id, ctx->policy_name,
				 ctx->policy_type);
				...
				struct update_group *updgrp)
{
	...
	if (old_subgrp->peer_count == 1) {
		...
		if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
			zlog_debug("u%" PRIu64 ":s%" PRIu64 " old peer count:%d"
	...
	if (BGP_DEBUG(update_groups, UPDATE_GROUPS))
		zlog_debug("u%" PRIu64 ":s%" PRIu64
			   " old peer count:%d peer %s split and moved into u%" PRIu64 ":s%" PRIu64,
			   paf->subgroup->update_group->id, paf->subgroup->id,
			   old_subgrp->peer_count, paf->peer->host, updgrp->id, subgrp->id);
	frrtrace(6, frr_bgp, ug_subgroup_split_peer, paf->subgroup->update_group->id,
		 paf->subgroup->id, old_subgrp->peer_count, paf->peer->host, updgrp->id, subgrp->id);
...
void peer_af_announce_route(struct peer_af *paf, int combine)
{
	...
	if (!combine || !all_pending) {
		...
		if (bgp_debug_update(paf->peer, NULL, subgrp->update_group, 0))
			zlog_debug("u%" PRIu64 ":s%" PRIu64" %s announcing routes",
				   subgrp->update_group->id, subgrp->id,
				   paf->peer->host);
		frrtrace(3, frr_bgp, upd_peer_af_announce_route,
	...
	frrtrace(4, frr_bgp, upd_peer_af_announce_route_combined,
		 subgrp->update_group->id, subgrp->id, paf->peer->host,
		 subgrp->peer_count);
