					  ...
					  struct prefix_rd *prd)
{
	...
	if (aspath_loop_check(path_vpn->attr->aspath, to_bgp->as)) {
		...
		if (bpi) {
			if (debug)
				zlog_debug("%s: blocking import of %p, as-path match", __func__,
					   bpi);
			bgp_aggregate_decrement(to_bgp, p, bpi, afi, safi);
			bgp_path_info_delete(bn, bpi);
			bgp_process(to_bgp, bn, bpi, afi, safi);
	...
	/* Certain switch AISC do not have capability for vrf leaked routes
	 ...
	 */
#ifdef ENABLE_ASIC_SUPPORT_VRF_LEAK
