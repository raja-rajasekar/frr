		   ...
		   bgp_size_t length)
{
	...
	char attr_str1[BUFSIZ] = {0};

	bgp_dump_attr(attr, attr_str1, sizeof(attr_str));
	if (bgp_debug_update(peer, NULL, NULL, 1))
		zlog_debug("%s: attributes: %s", __func__, attr_str1);

	frrtrace(2, frr_bgp, upd_malformed_attr, peer->host, attr_str1);
...
#define LEN_LEFT	(length - (stream_get_getp(s) - start))
	...
	case BGP_ATTR_NHLEN_VPNV6_GLOBAL_AND_LL:
		...
		if (!IN6_IS_ADDR_LINKLOCAL(&attr->mp_nexthop_local)) {
			...
			char addrgbl[BUFSIZ];
			char addrlocal[BUFSIZ];

			frrtrace(3, frr_bgp, upd_ignoring_non_ll_nexthop,
				 peer->host,
	...
	if (type == BGP_PREFIX_SID_SRV6_L3_SERVICE_SID_STRUCTURE) {
		...
		if (bgp_debug_update(peer, NULL, NULL, 1))
			zlog_debug(
				...
				peer->host, type);
		frrtrace(2, frr_bgp, upd_attr_type_unsupported, 1, peer->host,
			 type);
...
bgp_attr_srv6_service(struct bgp_attr_parser_args *args)
{
	...
	/* Placeholder code for unsupported type */
		...
		frrtrace(3, frr_bgp, upd_attr_type_unsupported, 2, peer->host,
			 type);
		  ...
		  struct bgp_attr_parser_args *args)
{
	...
	/* Placeholder code for Unsupported TLV */
		...
		frrtrace(3, frr_bgp, upd_attr_type_unsupported, 3, peer->host,
			 type);
