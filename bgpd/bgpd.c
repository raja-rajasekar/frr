...
static struct bgp *bgp_create(as_t *as, const char *name, enum bgp_instance_type inst_type,
			      const char *as_pretty, enum asnotation_mode asnotation,
...
peer_init:
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++) {
		...
		if (!hidden) {
			...
			bgp->vpn_policy[afi].export_vrf->del = bgp_vrf_string_name_delete;
		}
	...
	if (bgp) {
		...
		if (bgp->as != *as) {
			if (hidden || CHECK_FLAG(bgp->vrf_flags, BGP_VRF_AUTO)) {
				if (hidden) {
					bgp_create(as, name, inst_type, as_pretty, asnotation, bgp,
						   hidden);
		...
		if (bgp->inst_type != inst_type)
			return BGP_ERR_INSTANCE_MISMATCH;
		if (hidden)
			bgp_create(as, name, inst_type, as_pretty, asnotation, bgp, hidden);
		*bgp_val = bgp;
		return BGP_INSTANCE_EXISTS;
	}
	*bgp_val = NULL;
	    ...
	    enum asnotation_mode asnotation)
{
	...
	ret = bgp_lookup_by_as_name_type(bgp_val, as, as_pretty, asnotation, name, inst_type);
	if (ret || *bgp_val)
		return ret;

	bgp = bgp_create(as, name, inst_type, as_pretty, asnotation, NULL, false);
...
int bgp_delete(struct bgp *bgp)
{
	...
	if (bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT &&
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("Marking the deleting default bgp instance as hidden");
