...
struct bgp_show_peer_args {
	uint8_t max_neighbor_width;
	uint16_t sh_flags;
};
...
DEFPY(neighbor_graceful_shutdown, neighbor_graceful_shutdown_cmd,
      "[no$no] neighbor <A.B.C.D|X:X::X:X|WORD>$neighbor graceful-shutdown",
      NO_STR NEIGHBOR_STR NEIGHBOR_ADDR_STR2 "Graceful shutdown\n")
{
	int ret;
...
DEFPY(no_bgp_per_src_nhg_convergence_timer, no_bgp_per_src_nhg_convergence_timer_cmd,
...
static void bgp_show_bestpath(struct vty *vty, struct bgp *bgp, json_object *json)
{
	...
	if (use_json) {
		...
		if (CHECK_FLAG(bgp->flags, BGP_FLAG_ASPATH_MULTIPATH_RELAX)) {
			json_object_boolean_true_add(bestpath, "asPathMultiPathRelaxEnabled");
			if (CHECK_FLAG(bgp->flags, BGP_FLAG_MULTIPATH_RELAX_AS_SET))
				json_object_string_add(bestpath, "asPathMultiPathRelax", "as-set");
		} else {
			json_object_boolean_false_add(bestpath, "asPathMultiPathRelaxEnabled");
		}
		...
		if (bgp->lb_handling == BGP_LINK_BW_IGNORE_BW)
			...
			json_object_string_add(bestpath, "linkBandwidth", "skip-missing");
		...
		if (CHECK_FLAG(bgp->flags, BGP_FLAG_ASPATH_MULTIPATH_RELAX)) {
			if (CHECK_FLAG(bgp->flags, BGP_FLAG_MULTIPATH_RELAX_AS_SET))
				vty_out(vty, "  AS path multi-path-relax: as-set\n");
			else
				vty_out(vty, "  AS path multi-path-relax is Enabled\n");
		} else {
			vty_out(vty, "  AS path multi-path-relax is Disabled\n");
		}
		...
		vty_out(vty, "  Compare router ID is %s\n",
			CHECK_FLAG(bgp->flags, BGP_FLAG_COMPARE_ROUTER_ID) ? "Enabled" : "Disabled");
		...
		if (bgp->lb_handling == BGP_LINK_BW_IGNORE_BW)
			vty_out(vty, "  Link Bandwidth handling set to: ignore\n");
		else if (bgp->lb_handling == BGP_LINK_BW_SKIP_MISSING)
			vty_out(vty, "  Link Bandwidth handling set to: skip-missing\n");
		else if (bgp->lb_handling == BGP_LINK_BW_DEFWT_4_MISSING)
			vty_out(vty,
				"  Link Bandwidth handling set to: default-weight-for-missing\n");
		else
			vty_out(vty, "  Link Bandwidth handling set to: ecmp(default)\n");
		...
		vty_out(vty, "  Deterministic MED is %s\n",
			CHECK_FLAG(bgp->flags, BGP_FLAG_DETERMINISTIC_MED) ? "Enabled" : "Disabled");
	}
}
		       ...
		       "JSON output\n")
{
	...
	/* Handle single VRF case */
	if (name && !all) {
		bgp = strmatch(name, VRF_DEFAULT_NAME) ? bgp_get_default()
		...
		/* Output single VRF info */
		if (uj) {
			json_vrf = json_object_new_object();
			bgp_show_bestpath(vty, bgp, json_vrf);
			json_object_object_add(json, name, json_vrf);
			vty_json(vty, json);
		} else {
			vty_out(vty, "VRF %s\n", name);
...
static void bgp_show_peer(struct vty *vty, struct peer *p, void *args, bool use_json,
			  json_object *json)
{
	struct bgp *bgp;
	...
	struct bgp_show_peer_args *sh_args = (struct bgp_show_peer_args *)args;
	int neighbor_col_default_width = sh_args->max_neighbor_width;
	bool show_brief = ((CHECK_FLAG(sh_args->sh_flags, VTY_BGP_PEER_SHOW_STATE_ESTABLISHED_INFO) ||
			    CHECK_FLAG(sh_args->sh_flags, VTY_BGP_PEER_SHOW_STATE_FAILED_INFO) ||
			    CHECK_FLAG(sh_args->sh_flags, VTY_BGP_PEER_SHOW_BRIEF_INFO)));
	...
	if (show_brief) {
		if (use_json) {
			...
			len = vty_out(vty, "%10u %9u %9u %10s %12s ", p->as, PEER_TOTAL_RX(p),
				      PEER_TOTAL_TX(p),
	...
	if (sh_args->sh_flags & VTY_BGP_PEER_SHOW_GR_INFO) {
			     ...
			     bool use_json, json_object *json)
{
	...
	int find = 0;
	bool nbr_output = false;
	struct bgp_show_peer_args show_peer_args = {
		.max_neighbor_width = 0,
		.sh_flags = sh_flags,
	};
	int neighbor_col_default_width = 16;
	afi_t afi = AFI_MAX;
	...
	/* Find max neighbor length for formatting */
	int max_neighbor_width = neighbor_col_default_width;
	if (show_brief) {
		for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			int len;
			...
			if (peer->hostname && CHECK_FLAG(bgp->flags, BGP_FLAG_SHOW_HOSTNAME))
				// peer name displayed in the format
				// <hostname>(<host>)
				len = strlen(peer->hostname) + strlen(peer->host) + 2;
			else
	...
	show_peer_args.max_neighbor_width = max_neighbor_width;
	...
	for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
		...
		switch (type) {
		case show_all:
			bgp_show_peer(vty, peer, &show_peer_args, use_json, json);
			nbr_output = true;
			break;
		case show_peer:
			if (conf_if) {
				...
				if (peer->afc[afi][safi]) {
					if (conf_if) {
						if ((peer->conf_if
							...
							&& !strcmp(peer->hostname, conf_if))) {
							find = 1;
							bgp_show_peer(vty, peer, &show_peer_args,
								      use_json, json);
							break;
						}
					} else {
					 ...
					 struct peer *peer)
{
	...
	if (peergroup_flag_check(peer, PEER_FLAG_PASSWORD)) {
		if (host.obfuscate)
			caesar(true, peer->password, BGP_PASSWD_OBFUSCATION_KEY);
...
void bgp_vty_init(void)
{
	...
	install_element(BGP_EVPN_NODE, &neighbor_maximum_prefix_threshold_warning_cmd);
	install_element(BGP_EVPN_NODE, &neighbor_maximum_prefix_restart_cmd);
	install_element(BGP_EVPN_NODE, &neighbor_maximum_prefix_threshold_restart_cmd);
