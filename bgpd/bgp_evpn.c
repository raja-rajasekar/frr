		...
		for (dest = bgp_table_top(table); dest;
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi;
				...
				if (old_soo_fnd && !new_soo_fnd) {
					...
					prefix2str(evp, prefix_str,
						   sizeof(prefix_str));
					frrtrace(2, frr_bgp,
						 upd_soo_change_reimport_prefix,
						 prefix_str, attr_str);
	...
	void *new_martian)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bgp->peer, node, peer)) {
		...
		if (CHECK_FLAG(peer->af_flags[afi][safi],
			...
			frrtrace(3, frr_bgp, upd_evpn_martian_change,
				 peer->host, bgp_martian_type2str(martian_type),
				 1);
...
vni_t bgp_evpn_path_info_get_l3vni(const struct bgp_path_info *pi)
{
	...
	mpls_label_t *label = bgp_evpn_path_info_labels_get_l3vni(pi->extra->label,
								  pi->extra->num_labels);
	if (!label) {
		const struct prefix *dest_p = bgp_dest_get_prefix(pi->net);
		/* Shouldn't happen: label not found in pi->extra */
		flog_err(EC_BGP_PATH_WITHOUT_LABEL,
			 "%s: %pFX has path_info without label stack, label=%p, num_labels=%d",
			 __func__, dest_p, pi->extra->label, pi->extra->num_labels);
		return 0;
	}
