			...
			for (; pi; pi = pi->next) {
				...
				if (!brief) {
					...
					if (type == bgp_show_type_lcommunity) {
						...
						if (!bgp_attr_get_lcommunity(pi->attr) ||
						    !lcommunity_match(bgp_attr_get_lcommunity(
									      pi->attr),
								      lcom))
							continue;
					}
				...
				if (!brief) {
					...
					if (option == SHOW_DISPLAY_TAGS)
						route_vty_out_tag(vty, bgp_dest_get_prefix(rm), pi,
								  no_display, SAFI_EVPN, json_array);
					else if (option == SHOW_DISPLAY_OVERLAY)
				...
				if (no_display) {
					...
					if (best_path_selected)
						...
						json_object_boolean_false_add(json_flags,
									      "bestPathExists");
					json_object_object_add(json_prefix_info, "flags",
							       json_flags);
					json_object_int_add(json_nroute, "numPrefixes",
							    rd_prefix_cnt);

					json_object_object_addf(json_nroute, json_prefix_info,
								"%pFX", p);
				}
				json_array = NULL;
				json_flags = NULL;
      ...
      "Name of the route map\n")
{
	...
	return bgp_static_set(vty, bgp, false, argv[idx_ipv4_prefixlen]->arg,
			      argv[idx_route_distinguisher]->arg, argv[idx_label]->arg, AFI_L2VPN,
			      SAFI_EVPN, NULL, 0, 0, BGP_EVPN_IP_PREFIX_ROUTE, argv[idx_esi]->arg,
			      argv[idx_gwip]->arg, argv[idx_ethtag]->arg, argv[idx_routermac]->arg,
			      false, true);
}
      ...
      "Gateway IP\n" "Gateway IP ( A.B.C.D )\n" "Gateway IPv6 ( X:X::X:X )\n")
{
	...
	return bgp_static_set(vty, bgp, true, argv[idx_ipv4_prefixlen]->arg,
			      argv[idx_ext_community]->arg, argv[idx_label]->arg, AFI_L2VPN,
			      SAFI_EVPN, NULL, 0, 0, BGP_EVPN_IP_PREFIX_ROUTE, argv[idx_esi]->arg,
			      argv[idx_gwip]->arg, argv[idx_ethtag]->arg, NULL, false, true);
}
			       ...
			       json_object *json, bool brief)
{
	...
	for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
		...
		if (pi) {
			...
			if (!brief)
				/* Prefix and num paths displayed once per
				 * prefix. */
...
DEFPY(show_ip_bgp_l2vpn_evpn_neighbor_routes,
      ...
      "show [ip] bgp l2vpn evpn neighbors <A.B.C.D|X:X::X:X|WORD> routes [<brief$brief>] [json]",
      SHOW_STR
      IP_STR
      BGP_STR
      L2VPN_HELP_STR
      ...
      JSON_STR)
{
	...
	return bgp_show_ethernet_vpn(vty, NULL, bgp_show_type_neighbor, peer, SHOW_DISPLAY_STANDARD,
				     uj, brief);
}
       ...
       "Withdraw All default g/w mac-ip routes from EVPN\n")
{
	...
	if (!IS_EVPN_CONFIGURABLE(bgp)) {
		vty_out(vty, "This command is only supported under the EVPN VRF\n");
		return CMD_WARNING;
	}
...
DEFPY(show_bgp_l2vpn_evpn_route,
      ...
      "show bgp l2vpn evpn route [detail] [type " EVPN_TYPE_ALL_LIST "] [" BGP_SELF_ORIG_CMD_STR "] [<brief$brief>] [json]",
      SHOW_STR
      BGP_STR
      L2VPN_HELP_STR
      EVPN_HELP_STR
      ...
      JSON_STR)
{
	...
	evpn_show_all_routes(vty, bgp, type, json, detail, self_orig, brief);
...
DEFPY(show_bgp_l2vpn_evpn_route_rd,
      ...
      "show bgp l2vpn evpn route rd <ASN:NN_OR_IP-ADDRESS:NN|all> [type " EVPN_TYPE_ALL_LIST"] [<brief$brief>] [json]",
      SHOW_STR
      BGP_STR
      L2VPN_HELP_STR
      EVPN_HELP_STR
      ...
      JSON_STR)
{
	...
	if (rd_all) {
		...
		evpn_show_route_rd(vty, bgp, &prd, type, json, brief);
	}
