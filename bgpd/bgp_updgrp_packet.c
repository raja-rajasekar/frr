...
struct bpacket *subgroup_withdraw_packet(struct update_subgroup *subgrp)
{
	...
	while ((adv = bgp_adv_fifo_first(&subgrp->sync->withdraw)) != NULL) {
		...
		if (afi == AFI_IP && safi == SAFI_UNICAST
			...
			if (first_time) {
				...
				frrtrace(4, frr_bgp, upd_send_mp_unreach,
					 subgrp->update_group->id, subgrp->id,
					 iana_afi2str(pkt_afi),
					 iana_safi2str(pkt_safi));
			}
	...
	if (!stream_empty(s)) {
		...
		frrtrace(4, frr_bgp, upd_send_withdraw_details,
			 subgrp->update_group->id, subgrp->id,
...
void subgroup_default_withdraw_packet(struct update_subgroup *subgrp)
{
	...
	frrtrace(3, frr_bgp, upd_send_withdraw_default_originate,
		 (SUBGRP_UPDGRP(subgrp))->id, subgrp->id, tx_id_buf);
