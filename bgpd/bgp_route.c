					...
					safi_t safi, uint32_t addpath_tx_id)
{
	...
	if (onlypeer && CHECK_FLAG(onlypeer->af_sflags[afi][safi], PEER_STATUS_ORF_WAIT_REFRESH)) {
		if (debug)
			zlog_debug("%s: p=%pFX, First update is deferred until ORF or ROUTE_REFRESH is received",
				   __func__, p);
		return;
	}
	...
	if (selected) {
		...
		if (announce) {
			...
			if (advertise) {
				 ...
				 * the best path
				 */
				withdrawal = bgp_check_withdrawal(bgp, dest, safi, selected);
				if (debug)
			...
			if (debug)
				zlog_debug("%s: p=%pFX, advertise %d , announce_check returned FALSE",
		...
		if (!CHECK_FLAG(pi->flags, BGP_PATH_REMOVED) && same_attr &&
			...
			} else /* Duplicate - odd */
			{
				...
				if (CHECK_FLAG(pi->flags, BGP_PATH_STALE)) {
					...
					bgp_dest_set_defer_flag(dest, false);
					bgp_process(bgp, dest, pi, afi, safi);
				} else if ((CHECK_FLAG(bgp->per_src_nhg_flags[afi][safi],
						       BGP_FLAG_NHG_PER_ORIGIN)) &&
					   (safi != SAFI_EVPN) && route_has_soo_attr(pi)) {
					bgp_process(bgp, dest, pi, afi, safi);
				}
			}
...
static void bgp_nexthop_reachability_check(afi_t afi, safi_t safi, struct bgp_path_info *bpi,
					   const struct prefix *p, struct bgp_dest *dest,
					   struct bgp *bgp, struct bgp *bgp_nexthop,
					   bool skip_import_check)
{
	/* Nexthop reachability check. */
	if (safi == SAFI_UNICAST || safi == SAFI_LABELED_UNICAST) {
		if (CHECK_FLAG(bgp->flags, BGP_FLAG_IMPORT_CHECK) && (skip_import_check == false)) {
...
void bgp_static_update(struct bgp *bgp, const struct prefix *p, struct bgp_static *bgp_static,
		       afi_t afi, safi_t safi, bool skip_import_check)
{
	struct bgp_dest *dest;
	struct bgp_path_info *pi;
	struct bgp_path_info *new;
...
int bgp_static_set(struct vty *vty, struct bgp *bgp, bool negate, const char *ip_str,
		   const char *rd_str, const char *label_str, afi_t afi, safi_t safi,
		   const char *rmap, int backdoor, uint32_t label_index, int evpn_type,
		   const char *esi, const char *gwip, const char *ethtag, const char *routermac,
		   bool skip_import_check, bool user_configured)
{
	...
	if (negate) {
		...
		if (!bgp_static->backdoor)
			bgp_static_update(bgp, &p, bgp_static, afi, safi, skip_import_check);
	}
				...
				for (rm = bgp_table_top(table); rm;
					...
					bgp_static =
						bgp_dest_get_bgp_static_info(
							rm);
					bgp_static_update(bgp, bgp_dest_get_prefix(rm), bgp_static,
							  afi, safi, false);
				}
			} else {
				bgp_static_update(bgp, bgp_dest_get_prefix(dest),
						  bgp_dest_get_bgp_static_info(dest), afi, safi,
						  false);
			}
		}
	...
	FOREACH_AFI_SAFI (afi, safi) {
		for (dest = bgp_table_top(bgp->route[afi][safi]); dest;
			...
			if ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP)
				...
				bgp_static = bgp_dest_get_bgp_static_info(dest);
				bgp_static_update(bgp, bgp_dest_get_prefix(dest), bgp_static, afi,
						  safi, false);
			}
		}
	}
	...
	"Specify a BGP backdoor route\n")
{
	...
	return bgp_static_set(vty, bgp, negate, address_str ? addr_prefix_str : prefix_str, NULL,
			      NULL, AFI_IP, bgp_node_safi(vty), map_name, backdoor ? 1 : 0,
			      label_index ? (uint32_t)label_index : BGP_INVALID_LABEL_INDEX, 0,
	...
	"Label index value\n")
{
	...
	return bgp_static_set(vty, bgp, negate, prefix_str, NULL, NULL, AFI_IP6, bgp_node_safi(vty),
			      map_name, 0,
			      label_index ? (uint32_t)label_index : BGP_INVALID_LABEL_INDEX, 0,
			      NULL, NULL, NULL, NULL, false, true);
}
	...
	if (json_path) {
		...
		if (CHECK_FLAG(path->flags, BGP_PATH_MULTIPATH)) {
			if (is_path_using_soo_nhg(p, path, &soo_nhg, &soo_ip)) {
				json_object_boolean_true_add(json_path, "multipathNhg");
			} else {
...
void route_vty_out(struct vty *vty, const struct prefix *p, struct bgp_path_info *path, int display,
		   safi_t safi, json_object *json_paths, bool wide, char *rd_str)
{
	int len;
	...
	if ((safi == SAFI_ENCAP) || (safi == SAFI_MPLS_VPN)) {
		...
		if (json_paths) {
			...
			if (nexthop_hostname)
				len = vty_out(vty, "%pI4 (%s)%s", &attr->mp_nexthop_global_in,
					      nexthop_hostname, vrf_id_str);
			else
				len = vty_out(vty, "%pI4 %s", &attr->mp_nexthop_global_in,
					      vrf_id_str);
			  ...
			  json_object *json_paths)
{
	...
	if ((p->family == AF_INET || p->family == AF_ETHERNET ||
		...
		if (safi == SAFI_MPLS_VPN || safi == SAFI_ENCAP
			...
			if (json_paths) {
				...
				if (nexthop_hostname)
					vty_out(vty, "    %pI4 (%s)", &attr->nexthop,
						nexthop_hostname);
				else
					vty_out(vty, "    %pI4",
						&attr->nexthop);
		...
		if (json_paths) {
			...
			if (nexthop_hostname)
				vty_out(vty, "    %pI6 (%s)", &attr->mp_nexthop_global,
					nexthop_hostname);
			else
				vty_out(vty, "    %pI6",
					&attr->mp_nexthop_global);
	...
	if (CHECK_FLAG(path->flags, BGP_PATH_MULTIPATH) ||
		...
		bool is_multipath_nhg = is_path_using_soo_nhg(p, path, &soo_nhg, &soo_ip);
		if (json_paths) {
			if (is_multipath_nhg) {
				json_object_boolean_true_add(json_path, "multipathSooNhgFlag");
			} else {
				json_object_boolean_true_add(json_path, "multipath");
			}
		} else {
	...
	for (dest = bgp_table_top(table); dest; dest = next) {
		...
		if (itable != NULL) {
			...
			bgp_show_table(vty, bgp, afi, safi, itable, type, output_arg, rd,
				       next == NULL, &output_cum, &total_cum, &json_header_depth,
				 ...
				 bool incremental_print)
{
	...
	if (safi == SAFI_EVPN) {
		...
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
			if (is_path_using_soo_nhg(p, pi, &soo_nhg, &soo)) {
				path_using_soo_nhg = true;
				break;
			}
		}
		...
		if (!json) {
			vty_out(vty, "BGP routing table entry for %s%s%pFX, version %" PRIu64,
				(((safi == SAFI_MPLS_VPN || safi == SAFI_ENCAP) && prd)
					 ? prefix_rd2str(prd, buf1, sizeof(buf1), bgp->asnotation)
					 : ""),
				safi == SAFI_MPLS_VPN && prd ? ":" : "", p, dest->version);
			if (path_using_soo_nhg) {
				vty_out(vty, " SoO:%pIA, multipath soo nhg:%d\n", &ip, soo_nhg);
			} else {
				vty_out(vty, "\n");
			}
			...
			if (incremental_print) {
				...
				json_object_int_add(json, "version",
						    dest->version);
				if (path_using_soo_nhg) {
					json_object_string_addf(json, "soO", "%pIA", &ip);
					json_object_int_add(json, "multipathSoONhgId", soo_nhg);
				}
			}
		}
	}
	...
	if (safi == SAFI_MPLS_VPN || safi == SAFI_ENCAP) {
		for (dest = bgp_table_top(rib); dest;
			...
			bgp_show_path_info((struct prefix_rd *)dest_p, rm, vty, bgp, afi, safi,
					   json, pathtype, &display, rpki_target_state);
       ...
       JSON_STR)
{
	...
	if (argv_find(argv, argc, "AA:BB:CC", &idx)) {
		...
		return bgp_show(vty, bgp, afi, safi, bgp_show_type_lcommunity_all, NULL, show_flags,
				RPKI_NOT_BEING_USED, false);
}
	...
	if (match) {
		...
		if (ret != RMAP_DENY) {
			...
			bgp_show_path_info(NULL /* prefix_rd */, dest, vty, bgp, afi, safi, json_net,
					   BGP_PATH_SHOW_ALL, &display, RPKI_NOT_BEING_USED);
			if (use_json)
					...
					if (ret != RMAP_DENY) {
						...
						if (detail) {
							if (use_json)
								json_net =
									json_object_new_object();
							bgp_show_path_info(NULL /* prefix_rd */,
									   dest, vty, bgp, afi,
									   safi, json_net,
									   BGP_PATH_SHOW_ALL,
									   &display,
			   ...
			   uint16_t show_flags, bool brief)
{
	...
	if (use_json) {
		if (type == bgp_show_adj_route_advertised ||
			...
			if (header1) {
				uint64_t version = table ? table->version : 0;
				vty_out(vty, "\"bgpTableVersion\":%llu", version);
	...
	if (use_json) {
		if (type == bgp_show_adj_route_advertised ||
			...
			/* for bgp_show_adj_route_filtered &
			 * bgp_show_adj_route_bestpath type */
...
DEFPY(show_ip_bgp_instance_neighbor_advertised_route,
      ...
      "show [ip] bgp [<view|vrf> VIEWVRFNAME] [" BGP_AFI_CMD_STR " [" BGP_SAFI_WITH_LABEL_CMD_STR
      "]] [all$all] neighbors <A.B.C.D|X:X::X:X|WORD> <advertised-routes|received-routes|filtered-routes> [route-map RMAP_NAME$route_map] [<A.B.C.D/M|X:X::X:X/M>$prefix | detail$detail | <brief$brief>] [json$uj | wide$wide]",
