...
static void bgp_evpn_local_es_flush_esr_and_ead_routes(struct bgp *bgp,
							struct bgp_evpn_es *es)
{
	struct prefix_evpn p;
	int ret;
						      ...
						      struct bgp *bgp_vrf)
{
	...
	es_vrf->nhg_id = bgp_nhg_id_alloc(EVPN_MH);
	es_vrf->v6_nhg_id = bgp_nhg_id_alloc(EVPN_MH);
...
static void bgp_evpn_es_vrf_delete(struct bgp_evpn_es_vrf *es_vrf)
{
	...
	bgp_evpn_l3nhg_deactivate(es_vrf);
	if (es_vrf->nhg_id)
		bgp_nhg_id_free(EVPN_MH, es_vrf->nhg_id);
	es_vrf->nhg_id = 0;
	if (es_vrf->v6_nhg_id)
		bgp_nhg_id_free(EVPN_MH, es_vrf->v6_nhg_id);
	es_vrf->v6_nhg_id = 0;
