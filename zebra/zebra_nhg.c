...
struct nhg_hash_entry *zebra_nhg_alloc(void)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("RAJA %s Creating the nhe %u (%p) re-tree", __func__, nhe->id, nhe);
...
void zebra_nhg_free(struct nhg_hash_entry *nhe)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("RAJA %s Emptying the nhe %u (%p) re-tree(total re entries %lu)",
			   __func__, nhe->id, nhe, nhe_re_tree_count(&nhe->re_head));
	...
	if (op == DPLANE_OP_NH_DELETE) {
		...
		case ZEBRA_DPLANE_REQUEST_SUCCESS:
			...
			if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_KERNEL_DEL)) {
				struct route_entry *rre = NULL;
				int i = 1;
				frr_each (nhe_re_tree, &nhe->re_head, rre) {
					zlog_debug("%s RAJA NHE %p (%pNG) head %p Installed. Now print re tree entries  %d. re %p",
						   __func__, nhe, nhe, &nhe->re_head, i, rre);
					i++;
				}
				...
				while (nhe_re_tree_count(&nhe->re_head)) {
					...
					if (IS_ZEBRA_DEBUG_NHG_DETAIL)
						zlog_debug("RAJA %s Route re-install for re %p (%pRN, %s) on nhe %u (%p)",
							   __func__, re, re->rn,
					 ...
					 * want to be installing this BGP route entry anymore.
					 */
