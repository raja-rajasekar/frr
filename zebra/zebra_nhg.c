...
static struct nhg_ctx *nhg_ctx_init(uint32_t id, struct nexthop *nh,
				    ...
				    afi_t afi, int type, uint16_t count,
				    struct nhg_resilience *resilience,
				    bool startup)
{
					  ...
					  afi_t rt_afi, int type)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: => nhe %p (%pNG), flags 0x%x", __func__, nhe, nhe, nhe->flags);
...
void zebra_nhg_free(struct nhg_hash_entry *nhe)
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		...
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("%s: nhe %p (%pNG), refcnt %d flags 0x%x", __func__, nhe, nhe,
				   nhe->refcnt, nhe->flags);
		else
			zlog_debug("%s: nhe %p (%pNG), refcnt %d, flags 0x%x, NH %pNHv", __func__,
				   nhe, nhe, nhe->refcnt, nhe->flags, nhe->nhg.nexthop);
	}
...
void zebra_nhg_hash_free(void *p)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		/* Group or singleton? */
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("%s: nhe %p (%u), refcnt %d flags 0x%x", __func__, nhe, nhe->id,
				   nhe->refcnt, nhe->flags);
		else
			zlog_debug("%s: nhe %p (%pNG), refcnt %d, flags 0x%x, NH %pNHv", __func__,
				   nhe, nhe, nhe->refcnt, nhe->flags, nhe->nhg.nexthop);
...
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: nhe %p (%pNG) %d => %d, flags 0x%x", __func__, nhe, nhe,
			   nhe->refcnt, nhe->refcnt - 1, nhe->flags);
...
void zebra_nhg_install_kernel(struct nhg_hash_entry *nhe, uint8_t type)
{
	...
	if ((type != ZEBRA_ROUTE_CONNECT && type != ZEBRA_ROUTE_KERNEL) &&
	...
	if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_VALID) &&
		...
		case ZEBRA_DPLANE_REQUEST_FAILURE:
			flog_err(EC_ZEBRA_DP_INSTALL_FAIL,
				 "Failed to install Nexthop ID (%pNG) into the kernel", nhe);
			break;
...
/* This function uninstalls NHG from dplane and free up the if no re
 * referencing it.
 * free_nhg: variable used to not free up nhg but rather just uinstall from
 * dplane.
 */
void zebra_nhg_uninstall_kernel(struct nhg_hash_entry *nhe, bool free_nhg)
{
	...
	if (free_nhg)
		...
		if (IS_ZEBRA_DEBUG_DPLANE_DETAIL || IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s skip nhe %d free as there are re referencing it", __func__,
				   nhe->id);
	}
}
...
struct nhg_hash_entry *zebra_nhg_proto_del(uint32_t id, int type)
{
	...
	if (nhe->refcnt > 1) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("%s: %pNG, still being used by routes refcnt %u, flags 0x%x",
				   __func__, nhe, nhe->refcnt, nhe->flags);
		return nhe;
	}

	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: deleted nhe %p (%pNG), vrf %d, type %s, flags 0x%x", __func__, nhe,
			   nhe, nhe->vrf_id, zebra_route_string(nhe->type), nhe->flags);
