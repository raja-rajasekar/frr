	...
	struct nb_cb_get_elem_args *args)
{
	...
	return yang_data_new_date_and_time(args->xpath, UPTIMESECS(re->uptime));
}
...
{
	struct vrf *vrfp = (struct vrf *)args->list_entry;
	uint32_t route_count = lib_get_route_count(AFI_IP, SAFI_UNICAST, vrfp->vrf_id,
						   ZEBRA_ROUTE_CONNECT);
...
{
	struct vrf *vrfp = (struct vrf *)args->list_entry;
	uint32_t route_count = lib_get_route_count(AFI_IP6, SAFI_UNICAST, vrfp->vrf_id,
						   ZEBRA_ROUTE_CONNECT);
	return yang_data_new_uint32(args->xpath, route_count);
}
