...
DEFINE_HOOK(zebra_if_extra_info, (struct vty * vty, struct interface *ifp),
	    (vty, ifp));
DEFINE_HOOK(zebra_if_extra_info_json, (struct vty * vty, json_object *json, struct interface *ifp),
	    (vty, json, ifp));
...
static bool if_ignore_set_protodown(struct interface *ifp, bool new_down, uint32_t new_protodown_rc)
{
	struct zebra_if *zif;
...
bool zebra_if_get_source(struct interface *ifp, const struct ipaddr *dest, struct ipaddr *src)
{
	...
	frr_each_safe (if_connected, ifp->connected, ifc) {
		...
		} else if (dest->ipa_type == IPADDR_V6 && ifc->address->family == AF_INET6) {
			src->ipa_type = dest->ipa_type;
					...
					struct zebra_vxlan_vni *vni)
{
	...
	if (vty) {
		...
		if (vni->mcast_grp.s_addr != INADDR_ANY)
			vty_out(vty, "\n  Mcast Group %s", inet_ntoa(vni->mcast_grp));
	} else if (json_if) {
...
static void zebra_vxlan_if_dump_vty(struct vty *vty, struct zebra_if *zebra_if)
{
	...
	if (IS_ZEBRA_VXLAN_IF_VNI(zebra_if)) {
		...
		hash_iterate(vni_info->vni_table,
			     (void (*)(struct hash_bucket *, void *))zebra_vxlan_if_vni_hash_dump_vty,
			     args);
	}
			     ...
			     json_object *json)
{
	...
	if (IS_ZEBRA_IF_BRIDGE(ifp)) {
		...
		if (gre_info->ifindex_link && (gre_info->link_nsid != NS_UNKNOWN)) {
			struct interface *ifp;
	...
	json_object_boolean_add(json_if, "neighborThrottleDisable",
