...
void dplane_ctx_set_ifp_promiscuity(struct zebra_dplane_ctx *ctx, bool promiscuity)
{
	DPLANE_CTX_VALID(ctx);

	ctx->u.intf.promiscuity = promiscuity;
}

bool dplane_ctx_get_ifp_promiscuity(const struct zebra_dplane_ctx *ctx)
{
	DPLANE_CTX_VALID(ctx);

	return ctx->u.intf.promiscuity;
}
 ...
 */
static enum zebra_dplane_result dplane_route_update_last_internal(struct route_node *rn,
								  struct route_entry *re,
								  struct route_entry *old_re,
								  enum dplane_op_e op)
{
	...
	if (ret == AOK)
		...
		atomic_fetch_add_explicit(&zdplane_info.dg_route_errors, 1, memory_order_relaxed);
		if (ctx)
			dplane_ctx_free(&ctx);
	}
						  ...
						  struct route_entry *old_re)
{
	...
	ret = dplane_route_update_last_internal(rn, re, old_re, DPLANE_OP_ROUTE_LAST);
done:
	return ret;
}
