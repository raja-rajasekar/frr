					     ...
					     struct zebra_mac *mac)
{
	if ((wctx->flags & DEL_LOCAL_MAC) && (mac->flags & ZEBRA_MAC_LOCAL))
		return true;
	else if ((wctx->flags & DEL_REMOTE_MAC) && (mac->flags & ZEBRA_MAC_REMOTE)) {
	...
	} else if ((wctx->flags & DEL_REMOTE_MAC_FROM_VTEP) && (mac->flags & ZEBRA_MAC_REMOTE) &&
...
static void zebra_evpn_mac_del_hash_entry(struct hash_bucket *bucket, void *arg)
{
	...
	if (zebra_evpn_check_mac_del_from_db(wctx, mac)) {
		if (wctx->upd_client && (mac->flags & ZEBRA_MAC_LOCAL)) {
			zebra_evpn_mac_send_del_to_client(wctx->zevpn, &mac->macaddr, mac->flags,
							  false);
		}
		if (wctx->uninstall) {
 ...
 */
void zebra_evpn_mac_del_all(struct zebra_evpn *zevpn, int uninstall, int upd_client, uint32_t flags,
				      ...
				      uint32_t flags, bool force)
{
	...
	return zebra_evpn_macip_send_msg_to_client(zevpn->vni, macaddr, NULL, 0 /* flags */,
						   0 /* seq */, state, NULL, ZEBRA_MACIP_DEL);
}
					   ...
					   bool new_bgp_ready)
{
	if (new_bgp_ready)
		...
		zebra_evpn_mac_send_del_to_client(mac->zevpn, &mac->macaddr, mac->flags,
						  true /* force */);
}
				    ...
				    uint32_t seq, const esi_t *esi)
{
	...
	if (update_mac) {
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			if (CHECK_FLAG(mac->flags, ZEBRA_MAC_STICKY)) {
				zlog_warn(
					"Received remote mac add for MAC %pEA VNI %u VTEP %pI4, "
