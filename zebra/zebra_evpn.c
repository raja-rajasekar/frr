	...
	if (zevpn->advertise_gw_macip) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("VNI: %u GW-MACIP enabled, retain gw-macip",
				   zevpn->vni);
		frrtrace(1, frr_zebra, zebra_evpn_gw_macip_del_for_evpn_hash, zevpn);
		return;
	}
					    ...
					    void *ctxt)
{
	...
	frrtrace(1, frr_zebra, zebra_evpn_svi_macip_del_for_evpn_hash, zevpn);
...
void zebra_evpn_read_mac_neigh(struct zebra_evpn *zevpn, struct interface *ifp)
{
	...
	if (vlan_if) {
		...
		if (zevpn && br_if && vlan_if) {
			frrtrace(4, frr_zebra, zebra_evpn_read_mac_neigh, zevpn, br_if, vni,
				 vlan_if);
		}
...
int zebra_evpn_send_add_to_client(struct zebra_evpn *zevpn)
{
	...
	frrtrace(2, frr_zebra, zebra_evpn_send_add_to_client, zevpn, client);
...
int zebra_evpn_send_del_to_client(struct zebra_evpn *zevpn)
{
	...
	frrtrace(2, frr_zebra, zebra_evpn_send_del_to_client, client, zevpn);
	client->vnidel_cnt++;
	return zserv_send_message(client, s);
}
	...
	if (sticky || remote_gw) {
		if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_NEIGH
			...
			zlog_debug(
				...
				remote_gw ? " remote_gw" : "");
		if (macaddr && ipaddr) {
			frrtrace(7, frr_zebra, zebra_evpn_process_sync_macip_add, zevpn, macaddr,
				 ipaddr, ipa_len, esi, flags, seq);
		}
		return;
	}
			      ...
			      struct in_addr vtep_ip)
{
	...
	if (!ifp || !if_is_operative(ifp) || !zif || !zif->brslave_info.br_if) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				...
				vni);
		if (macaddr && ipaddr)
			frrtrace(4, frr_zebra, process_remote_macip_del, vni, macaddr, ipaddr,
				 vtep_ip);
		return;
	}
	...
	if (n)
		...
		if (zvrf->dad_freeze
			...
			if (macaddr && ipaddr)
				frrtrace(4, frr_zebra, process_remote_macip_del, vni, macaddr,
					 ipaddr, vtep_ip);
		}
