 ...
 */
static struct client_gr_info *zebra_gr_delete_stale_client(struct client_gr_info *info)
{
...
void zread_client_capabilities(ZAPI_HANDLER_ARGS)
{
	...
	case ZEBRA_CLIENT_ROUTE_UPDATE_COMPLETE:
		...
		rib_add_gr_run(api.afi, api.vrf_id, client->proto, client->instance,
			       client->restart_time, client->update_pending_time);
...
static void zebra_gr_route_stale_delete_timer_expiry(struct event *thread)
{
	...
	if (cnt > 0) {
		...
		LOG_GR("GR %s: Client %s vrf %s(%u) all stale routes processed", __func__,
