		...
		if (ifp && if_is_operative(ifp)) {
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH) {
				...
				if (ifp)
					...
					snprintf(if_name_buf, sizeof(if_name_buf), "ifindex %d", n->ifindex);

				zlog_debug(
						"sync-neigh vni %u ip %pIA DEL: ifp %s (idx %d) is not operative, not starting hold_timer for ES_PEER_ACTIVE flag 0x%x",
					    ...
					    void *arg)
{
	...
	if (((wctx->flags & DEL_LOCAL_NEIGH) && (n->flags & ZEBRA_NEIGH_LOCAL))
		...
		/*
		 * If we are doing stale cleanup of remote neighs but
		 * but if this neigh is not marked stale, the don't delete
		 * it
		 */
