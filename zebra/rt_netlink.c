...
{
	if ((proto == RTPROT_BGP) || (proto == RTPROT_OSPF) || (proto == RTPROT_ZSTATIC) ||
	    (proto == RTPROT_ZEBRA) || (proto == RTPROT_ISIS) || (proto == RTPROT_RIPNG) ||
	    (proto == RTPROT_NHRP) || (proto == RTPROT_EIGRP) || (proto == RTPROT_LDP) ||
	    (proto == RTPROT_BABEL) || (proto == RTPROT_RIP) || (proto == RTPROT_SHARP) ||
	    (proto == RTPROT_PBR) || (proto == RTPROT_OPENFABRIC) || (proto == RTPROT_SRTE) ||
	    (proto == RTPROT_FRR)) {
		return true;
	}
				   ...
				   void *buf, size_t buflen, bool fpm)
{
	...
	if (!fpm && !kernel_nexthops_supported()) {
		...
		frrtrace(3, frr_zebra, netlink_nexthop_msg_encode_err, id, zebra_route_string(type),
			 1);
		return 0;
	}
 ...
 */
static ssize_t kernel_br_port_update_ctx(struct zebra_dplane_ctx *ctx, void *buf, size_t buflen)
{
	struct {
		struct nlmsghdr n;
		struct ifinfomsg ifm;
	...
	nest = nl_attr_nest(&req->n, buflen, IFLA_PROTINFO | NLA_F_NESTED);
	if (nest == NULL)
		return 0;

	/* backup NHG */
	if (!nl_attr_put32(&req->n, buflen, IFLA_BRPORT_DUMMY_BACKUP_NHID, nhg_id))
		return 0;

	/* non-DF BUM block filter */
	block_bum = (flags & DPLANE_BR_PORT_NON_DF) ? 1 : 0;
	if (!nl_attr_put(&req->n, buflen, IFLA_BRPORT_DUMMY_BLOCK_BUM, &block_bum,
			 sizeof(block_bum)))
		return 0;
...
enum netlink_msg_status netlink_put_br_port_update_msg(struct nl_batch *bth,
						       struct zebra_dplane_ctx *ctx)
{
	return netlink_batch_add_msg(bth, ctx, kernel_br_port_update_ctx, false);
}
 ...
 */
static bool netlink_handle_5549(struct ndmsg *ndm, struct zebra_if *zif, struct interface *ifp,
				struct ipaddr *ip, bool handle_failed)
{
 ...
 */
static bool send_nd_helper(const struct ipaddr *addr, struct zebra_ns *zns, struct interface *ifp)
{
	uint8_t buf[200] = {};
...
#define ZEBRA_ND_SIZE                                                                              \
	ETHER_HDR_LEN + sizeof(struct ip6_hdr) + sizeof(struct nd_neighbor_advert) +               \
	...
	/* Compute checksum */
	hlen = (sizeof(struct nd_neighbor_advert) + sizeof(struct nd_opt_hdr) + ETH_ALEN);
	...
	len = sendto(zns->nd_fd, buf, ZEBRA_ND_SIZE, 0, (struct sockaddr *)&sll, sizeof(sll));
	if (len < 0) {
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("%s: error sending ND SOLICIT req for %pIA", __func__, addr);
		return false;
	}
 ...
 */
static bool send_arp_helper(const struct ipaddr *addr, struct zebra_ns *zns, struct interface *ifp)
{
	uint8_t buf[100];
	uint8_t *arp_ptr;
...
static int netlink_ipneigh_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if (h->nlmsg_type == RTM_GETNEIGH) {
		...
		netlink_handle_neigh_throttle(h->nlmsg_type, ndm, &ip, zns, ifp);
