...
static int do_show_ip_route(struct vty *vty, const char *vrf_name, afi_t afi, safi_t safi,
			    bool use_fib, json_object *vrf_json, bool use_json, route_tag_t tag,
       ...
       JSON_STR)
{
	...
	do_show_ip_route(vty, VRF_DEFAULT_NAME, ip ? AFI_IP : AFI_IP6, SAFI_MULTICAST, false, NULL,
...
static void vty_show_ip_route(struct vty *vty, struct route_node *rn, struct route_entry *re,
			      json_object *json, bool is_fib, bool show_ng, bool brief)
{
	const struct nexthop *nexthop;
	int len = 0;
	char buf[SRCDEST2STR_BUFFER];
	...
	if (json) {
		...
		json_object_int_add(json_route, "routeUptimeEstablishedEpoch", epoch_tbuf);
		if (brief) {
...
static void do_show_route_helper(struct vty *vty, struct zebra_vrf *zvrf, struct route_table *table,
				 afi_t afi, bool use_fib, json_object *vrf_json, route_tag_t tag,
				 const struct prefix *longer_prefix_p, bool supernets_only, int type,
				 unsigned short ospf_instance_id, bool use_json, uint32_t tableid,
				 ...
				 struct route_show_ctx *ctx)
{
	...
	RB_FOREACH (zrt, zebra_router_table_head,
		...
		do_show_ip_route(vty, zvrf_name(zvrf), afi, SAFI_UNICAST, use_fib, vrf_json,
				 use_json, tag, longer_prefix_p, supernets_only, type,
				 ospf_instance_id, zrt->tableid, show_ng, ctx, false);
	}
}
			    ...
			    struct route_show_ctx *ctx, bool brief)
{
	...
	do_show_route_helper(vty, zvrf, table, afi, use_fib, vrf_json, tag, longer_prefix_p,
      ...
      "Super Happy Advanced Routing Protocol (SHARP)\n" VRF_FULL_CMD_HELP_STR "Brief\n" JSON_STR)
{
	...
	show_nexthop_group_cmd_helper(vty, zvrf, afi, type, json, brief, &nhg_count);
...
DEFPY(show_route, show_route_cmd, "show\
         <\
	...
	if (vrf_all) {
		...
		RB_FOREACH (vrf, vrf_name_head, &vrfs_by_name) {
			...
			if (table_all)
				...
				do_show_ip_route(vty, zvrf_name(zvrf), afi, SAFI_UNICAST, !!fib,
						 vrf_json, !!json, tag, prefix_str ? prefix : NULL,
		...
		if (table_all)
			...
			do_show_ip_route(vty, vrf->name, afi, SAFI_UNICAST, !!fib, NULL, !!json,
					 tag, prefix_str ? prefix : NULL, !!supernets_only, type,
...
DEFPY(evpn_mh_tc_off, evpn_mh_tc_off_cmd, "[no$no] evpn mh tc-off",
      NO_STR "EVPN\n"
	     "Multihoming\n"
...
DEFUN(neigh_throttle_timeout, neigh_throttle_timeout_cmd,
      ZEBRA_NEIGH_THROTTLE_STR
      " timeout " CMD_RANGE_STR(ZEBRA_NEIGH_THROTTLE_MIN_TIMEOUT, ZEBRA_NEIGH_THROTTLE_MAX_TIMEOUT),
      "Neighbor data\n"
      "Neighbor throttling data\n"
...
DEFUN(no_neigh_throttle_timeout, no_neigh_throttle_timeout_cmd,
      "no " ZEBRA_NEIGH_THROTTLE_STR
      " timeout [" CMD_RANGE_STR(ZEBRA_NEIGH_THROTTLE_MIN_TIMEOUT,
				 ZEBRA_NEIGH_THROTTLE_MAX_TIMEOUT) "]",
      NO_STR "Neighbor data\n"
	     "Neighbor throttling data\n"
	     "Set timeout in seconds\n"
	     "Timeout in seconds\n")
...
DEFPY(neigh_throttle_limit, neigh_throttle_limit_cmd, ZEBRA_NEIGH_THROTTLE_STR " limit (0-100000)",
      "Neighbor data\n"
      "Neighbor throttling data\n"
...
DEFPY(no_neigh_throttle_limit, no_neigh_throttle_limit_cmd,
...
DEFPY(neigh_throttle_del, neigh_throttle_del_cmd,
...
DEFPY(show_evpn_es_peer, show_evpn_es_peer_cmd, "show evpn es-peer [json$json]",
      SHOW_STR "EVPN\n"
      ...
      SHOW_STR ZEBRA_STR "CSMgr information\n" JSON_STR)
{
	...
	if (uj) {
		json_object_boolean_add(json, "registeredWithCsm", zrouter.frr_csm_regd);
		...
		json_object_boolean_add(json, "csmLoadComplete", !zrouter.load_complete_failed);
	...
	if (uj) {
		json_object_int_add(json, "zebraUptime",
				    time_to_epoch(UPTIMESECS(zrouter.startup_time)));
		json_object_boolean_add(json, "zebraStartedGracefully", zrouter.graceful_restart);
	} else {
	...
	if (zrouter.t_rib_sweep) {
		...
		if (uj)
			json_object_int_add(json, "ribSweepTimerExpiry",
	...
	if (zrouter.t_gr_no_clients) {
		...
		if (uj)
			json_object_int_add(json, "grClientRegTimerExpiry",
	...
	if (zrouter.graceful_restart) {
		if (uj)
			json_object_boolean_add(json, "grComplete", zrouter.all_instances_gr_done);
		else
		...
		if (zrouter.all_instances_gr_done) {
			...
			if (uj)
				json_object_int_add(json, "grCompletionTime",
						    time_to_epoch(zrouter.gr_completion_time));
			else
