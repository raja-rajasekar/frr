					 ...
					 struct route_entry *re)
{
	...
	/* For connected route force trigger rnh (NHT)
	...
	if (prn) {
		...
		if (dest && !!CHECK_FLAG(dest->flags, RIB_DEST_FORCE_RNH)) {
			force = true;
			if (IS_ZEBRA_DEBUG_NHT_DETAILED)
				zlog_debug("%s %pRN is connected set force rnh to true", __func__,
					   prn);
		}
	}
		...
		for (nexthop = rnh->state->nhe->nhg.nexthop; nexthop;
			...
			if (json) {
				...
				json_object_array_add(json_nexthop_array,
						      json_nexthop);
				show_nexthop_json_helper(json_nexthop, nexthop, NULL, false);
			} else {
