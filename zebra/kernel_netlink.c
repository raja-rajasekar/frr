					  ...
					  struct zebra_dplane_ctx *ctx)
{
	...
	case DPLANE_OP_BR_PORT_UPDATE:
		return netlink_put_br_port_update_msg(bth, ctx);
...
void kernel_init(struct zebra_ns *zns)
{
	 ...
	 */
	frr_with_privs (&zserv_privs) {
		zns->arp_fd = socket(PF_PACKET, SOCK_RAW | SOCK_CLOEXEC, htons(ETH_P_ARP));
		if (zns->arp_fd < 0)
			zlog_warn("Failed to open ARP socket for NS %s.", zns->ns->name);

		zns->nd_fd = socket(PF_PACKET, SOCK_RAW | SOCK_CLOEXEC, htons(ETH_P_IPV6));
		if (zns->nd_fd < 0)
