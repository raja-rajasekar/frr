...
static void igmp_source_json_helper(struct gm_source *src, json_object *json_sources,
				    char *source_str, char *mmss, char *uptime,
				    time_t source_creation)
{
	...
	epoch_tbuf = time(NULL) - (monotime(NULL) - (UPTIMESECS(source_creation)));
			     ...
			     time_t now, bool detail)
{
	...
	if (uj) {
		...
		if (json_groups) {
			...
			json_object_int_add(json_group, "igmpGrpUptimeEpoch", epoch_tbuf);
			...
			if (detail) {
				...
				for (ALL_LIST_ELEMENTS_RO(
					...
					igmp_source_json_helper(src, json_sources, source_str, mmss,
								src_uptime, src->source_creation);
				}
			}
		}
	} else {
			       ...
			       json_object *json, struct vty *vty, bool uj)
{
	...
	if (uj) {
		...
		if (json_sources)
			igmp_source_json_helper(src, json_sources, source_str, mmss, uptime,
						src->source_creation);
	} else {
       ...
       JSON_STR)
{
	...
	if (uj) {
		...
		json_object_boolean_add(json, "mlagConnUp",
					CHECK_FLAG(router->mlag_flags, PIM_MLAGF_LOCAL_CONN_UP));
		json_object_boolean_add(json, "mlagPeerConnUp",
					CHECK_FLAG(router->mlag_flags, PIM_MLAGF_PEER_CONN_UP));
		json_object_boolean_add(json, "mlagPeerZebraUp",
					CHECK_FLAG(router->mlag_flags, PIM_MLAGF_PEER_ZEBRA_UP));
