...
bool HandleStreamingSubscriptionCache(
	StreamRpcState<frr::SubscriptionCacheRequest, frr::SubscriptionCacheResponse,
		       SubscriptionCacheContextType> *tag)
{
...
grpc::Status HandleUnarySubscribe(UnaryRpcState<frr::SubscribeRequest, frr::SubscribeResponse> *tag)
{
	grpc_debug("%s: entered", __func__);
	...
	for (const frr::Subscription &item : tag->request.subscribe().subscriptions()) {
		...
		grpc_debug("Subscribe %s(path: \"%s\") action: %s interval: %d", __func__,
			   xpath_str, action, interval);
	}
	return grpc::Status::OK;
}
